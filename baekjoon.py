...
# while문 반복으로 하니까 시간초과뜸..................
# 어차피 더하기 빼기 반복하는거니까 한번에 더하기만 해도 될 것 같긴한데 정상에서 더이상 미끄러지지 않는다는 조건이 있어서 더하기랑 빼기랑 나눴더니 시간초과가 뜨네...
'''
while True:
    day += 1
    location += A
...
print(day)
'''
# 이걸 일일히 더하는게 아니라 나누는걸로 한방에 해결을 해야할것같기도
# 근데 정상 도착하면 안미끄러진다니까 그거 고려는 어캐
# 주어진 길이에서 올라가는길이를 빼고, 그 수를 올라가는 길이로 나눠서 몫을 취하고 +1한걸 프린트하면 되지 않을까?

# 런타임에러라는데..? 왜......?
# 로직 오류: temp < move - temp가 음수일 때를 고려 (정확히 V와 A가 같아서 0으로 나누면 ZeroDivisionError 발생 → 런타임 에러 발생문제)
'''
move = A - B
temp = V - A
day = temp // move + 1
...
# 수정 후
# A > B이고, 한번 정상에 올라가면 그날은 미끄러지지 않기때문에 적어도 저 위치에 도달만 하면 다음날 더 내려가지 않는다.
# V - B까지는 매일 (A-B)씩 올라가고, 마지막 날 A만큼 올라감
move = A - B
temp = V - B - 1  #temp // move가 딱 나누어 떨어져 그것이 정답일때도 무조건 1을 더하기 때문에 -1을 해서 숫자를 내려줘야한다
day = temp // move + 1
...
# 하지만 그것마저 인풋이 21억까지 가야하기때문에 반복에는 한계가 있다.

# => 약수의 개수가 홀수인 것이 최종적으로 열림(1) 상태인 것인데, 
# 홀수인 경우는 제곱수의 경우 약수의 개수가 홀수이다.
# 제곱근: 제곱해서 a가 되는 수
...
'''
# 이건 n이 7일때만 해당
print((n-2) + (n-3)*2 + (n-4)*3 + (n-5)*4 + (n-6)*5)
print(3)
'''
...
# 하단이 문제가 아니라 모든 n>=n0에 대해 식이 성립해야함.
'''
...
# 충분히 큰 모든 n에 대해 성립하려면, a1 ≤ c 이어야 함 (계수 비교)
if g >= f:
    if  a1 <= c:
        print(1)
